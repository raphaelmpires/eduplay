...
const Translation = {
  init() {
    ...
    style.textContent = `
      ...
      .ep-translate-wrap.show-tooltip .ep-translate-btn, 
      .ep-translate-btn:hover {
        background: #1a6fc4;
        color: #fff;
        transform: scale(1.1);
  ...
  t(en, pt) {
    ...
    return `<span class="ep-translate-wrap"><span class="ep-translate-text">${en}</span><button class="ep-translate-btn" title="Traduzir" onclick="document.querySelectorAll('.ep-translate-wrap.show-tooltip').forEach(el => { if(el !== this.parentElement) el.classList.remove('show-tooltip'); }); this.parentElement.classList.toggle('show-tooltip'); event.stopPropagation(); event.preventDefault();">🌍</button><span class="ep-translate-tooltip">🇧🇷 ${safePt}</span></span>`;
  }
};
